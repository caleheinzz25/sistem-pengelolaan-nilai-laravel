...
<div class="space-y-6">
    ...
    <form action="{{ route('admin.guru.update', $guru) }}" method="POST" class="space-y-6 max-w-lg">
        @csrf @method('PUT')
        <div>
            <label for="kode_guru" class="block text-sm font-medium text-gray-900">Kode Guru</label>
            <input type="text" name="kode_guru" id="kode_guru" value="{{ old('kode_guru', $guru->kode_guru) }}" required
